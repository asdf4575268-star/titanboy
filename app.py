...
def make_smart_collage(files, target_size):
    ...
    for i, img in enumerate(imgs):
        ...
        curr_cols = cols if (r + 1) * cols <= n else remaining_in_row
...
if 'user_edited' not in st.session_state: st.session_state.user_edited = False
...
def reset_edit_flag(): st.session_state.user_edited = False
...
with col_main:
    ...
    mode = st.radio("모드 선택", ["DAILY", "WEEKLY", "MONTHLY"], horizontal=True, on_change=reset_edit_flag)
    ...
    if acts:
        with st.container(border=True):
            if mode == "DAILY":
                ...
                sel_act = st.selectbox("🏃 활동 선택", act_opts, on_change=reset_edit_flag)
                a = acts[act_opts.index(sel_act)]
                if not st.session_state.user_edited:
                    d_km = a.get('distance', 0)/1000; m_s = a.get('moving_time', 0)
                    st.session_state.ti_act, st.session_state.ti_date = a['name'], a['start_date_local'][:10]
                    st.session_state.ti_dist = f"{d_km:.2f}"
                    st.session_state.ti_time = f"{m_s//3600:02d}:{(m_s%3600)//60:02d}:{m_s%60:02d}"
                    st.session_state.ti_pace = f"{int((m_s/d_km)//60)}'{int((m_s/d_km)%60):02d}\"" if d_km > 0 else "0'00\""
                    st.session_state.ti_hr = str(int(a.get('average_heartrate', 0))) if a.get('average_heartrate') else "0"
            elif mode == "WEEKLY":
                ...
                sel_week = st.selectbox("📅 주차 선택", weeks, on_change=reset_edit_flag)
                weekly_data = get_weekly_stats(acts, sel_week.replace('.','-'))
                if weekly_data and not st.session_state.user_edited:
                    st.session_state.ti_act, st.session_state.ti_date = "WEEKLY RUN", weekly_data['range']
                    st.session_state.ti_dist, st.session_state.ti_time = weekly_data['total_dist'], weekly_data['total_time']
                    st.session_state.ti_pace, st.session_state.ti_hr = weekly_data['avg_pace'], weekly_data['avg_hr']
            elif mode == "MONTHLY":
                months = sorted(list(set([ac['start_date_local'][:7] for ac in acts])), reverse=True)
                sel_month = st.selectbox("🗓️ 월 선택", months, on_change=reset_edit_flag)
                monthly_data = get_monthly_stats(acts, f"{sel_month}-01")
                if monthly_data and not st.session_state.user_edited:
                    st.session_state.ti_act, st.session_state.ti_date = "MONTHLY RUN", monthly_data['range']
                    st.session_state.ti_dist, st.session_state.ti_time = monthly_data['total_dist'], monthly_data['total_time']
                    st.session_state.ti_pace, st.session_state.ti_hr = monthly_data['avg_pace'], monthly_data['avg_hr']

# --- [5. 오른쪽 디자인 창] ---
with col_design:
    st.header("🎨 DESIGN")
    def on_user_edit(): st.session_state.user_edited = True
    
    with st.expander("✍️ 텍스트 수정"):
        v_act = st.text_input("활동명", key="ti_act", on_change=on_user_edit)
        v_date = st.text_input("날짜", key="ti_date", on_change=on_user_edit)
        v_dist = st.text_input("거리 km", key="ti_dist", on_change=on_user_edit)
        v_time = st.text_input("시간", key="ti_time", on_change=on_user_edit)
        v_pace = st.text_input("페이스", key="ti_pace", on_change=on_user_edit)
        v_hr = st.text_input("심박 bpm", key="ti_hr", on_change=on_user_edit)
    ...
    with st.expander("📍 위치/크기 조절"):
        ...
        ry = st.number_input("박스 Y", 0, 1920, 1250 if mode=="DAILY" else 80)
        rw = st.number_input("박스 너비", 100, 1080, 1000 if box_orient=="Horizontal" else 450)
        rh = st.number_input("박스 높이", 100, 1920, 350 if box_orient=="Horizontal" else 550)
    ...
    if data_ready:
        try:
            ...
            if box_orient == "Vertical":
                ...
                draw_styled_text(draw, (rx+40, ry+110), v_date, f_d, "#AAAAAA", use_shadow)
                y_c = ry + 170
                ...
                draw_styled_text(draw, (rx + (rw - draw.textlength(v_act, font=f_t))//2, ry + 35), v_act, f_t, m_color, use_shadow)
                draw_styled_text(draw, (rx + (rw - draw.textlength(v_date, font=f_d))//2, ry + 115), v_date, f_d, "#AAAAAA", use_shadow)
                sec_w = rw // 4
                for i, (lab, val) in enumerate(items):
                    cx = rx + (i * sec_w) + (sec_w // 2)
                    draw_styled_text(draw, (cx - draw.textlength(lab.lower(), font=f_l)//2, ry + 175), lab.lower(), f_l, "#AAAAAA", use_shadow)
                    draw_styled_text(draw, (cx - draw.textlength(val.lower(), font=f_n)//2, ry + 215), val.lower(), f_n, sub_color, use_shadow)
            ...
            if st.checkbox("지도/그래프 보기", value=True):
                if mode == "DAILY" and a and a.get('map', {}).get('summary_polyline'):
                    ...
                    overlay.paste(vis_layer, (int(rx + (rw - vis_sz_adj)//2), int(ry - vis_sz_adj - 20)), vis_layer)
                elif mode in ["WEEKLY", "MONTHLY"]:
                    d_obj = weekly_data if mode == "WEEKLY" else monthly_data
                    if d_obj:
                        chart_img = create_bar_chart(d_obj['dists'], m_color, mode=mode, font_path=f_path)
                        v_l = chart_img.resize((vis_sz_adj, int(chart_img.size[1]*(vis_sz_adj/chart_img.size[0]))), Image.Resampling.LANCZOS)
            ...
            if log_file:
                ...
                overlay.paste(l_img, (rx + rw - ls - 25, ry + rh - ls - 25), l_img)
        ...
        st.info("🏃 Strava 데이터(활동 또는 주차)를 먼저 선택해주세요.")
