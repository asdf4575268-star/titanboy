...
# --- [5. 메인 레이아웃 구성 변경] ---
# 왼쪽(설정/데이터), 오른쪽(미리보기)으로 분할
col_config, col_preview = st.columns([1, 1.2], gap="large")
...
with col_config:
    ...
    with st.expander("🏃 데이터 선택/수정", expanded=True):
        ...
        # 수기 입력 필드 (사이드에 배치)
        v_act = st.text_input("활동명 직접 수정", value=v_act)
        c1, c2 = st.columns(2)
        v_dist = c1.text_input("거리(km)", value=v_dist)
        v_time = c2.text_input("시간", value=v_time)
        v_pace = c1.text_input("페이스", value=v_pace)
        v_hr = c2.text_input("심박(bpm)", value=v_hr)

    # 디자인 설정 섹션
    with st.expander("🎨 디자인 커스텀", expanded=False):
        ...
        vis_sz = st.slider("그래프/지도 크기", 50, 1000, 250 if mode=="DAILY" else 1000)
        vis_alpha = st.slider("그래프/지도 투명도", 0, 255, 180)
...
with col_preview:
    try:
        ...
        if log_file:
            ...
            overlay.paste(l_img, (1080 - ls - 30, ry + 30), l_img)

        # 최종 이미지 렌더링
        final = Image.alpha_composite(canvas, overlay).convert("RGB")
        st.image(final, use_container_width=True, caption="PREVIEW (1080px)")
        
        # 다운로드 버튼을 이미지 바로 아래에 배치
        buf = io.BytesIO(); final.save(buf, format="JPEG", quality=95)
        st.download_button(f"📥 {mode} 결과물 저장하기", buf.getvalue(), f"{mode.lower()}_{datetime.now().strftime('%Y%m%d')}.jpg", use_container_width=True)

    except Exception as e:
        st.info("데이터를 선택하면 미리보기가 생성됩니다.")
