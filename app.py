...
with col_main:
    ...
    if data_ready:
        try:
            ...
            # 2. 폰트 안전 로딩 (실패 시 기본 폰트로 즉시 우회)
            def get_f(n, s):
                try: return load_font(n, s)
                except: return ImageFont.load_default()

            f_t = get_f(sel_font, 90) # 활동명
            f_d = get_f(sel_font, 30) # 날짜
            f_n = get_f(sel_font, 60) # 숫자
            f_l = get_f(sel_font, 25) # 라벨

            # 3. 배경 이미지 생성
            final = make_smart_collage(bg_files, (CW, CH)) if bg_files else Image.new("RGB", (CW, CH), (20, 20, 20))
            ...
            if show_box:
                ...
                if box_orient == "Vertical":
                    ...
                    # 가로 모드 로직 (필요 시 기존 코드 유지)
                    pass

            # 5. 최종 합성 및 출력
            final_img = Image.alpha_composite(final, overlay).convert("RGB")
            ...
            st.error(f"렌더링 중 오류가 발생했습니다. 캐시를 삭제해 보세요: {e}")
