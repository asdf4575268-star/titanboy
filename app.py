...
def make_smart_collage(files, target_size):
    ...
    for i, img in enumerate(imgs):
        ...
        x1 = int(((i % cols) + 1) * (tw / (n % cols if (r == rows - 1 and n % cols != 0) else cols))) if r == rows - 1 and n % cols != 0 else int((c + 1) * tw / cols)
        y1 = int((r + 1) * th / rows)
...
with col_design:
    ...
    with st.expander("💄 매거진 스타일", expanded=True):
        show_vis, show_box, use_shadow = st.toggle("지도/그래프 표시", True), st.toggle("데이터 박스 표시", True), st.toggle("글자 그림자 효과", True)
        border_thick = st.slider("프레임 테두리 두께", 0, 50, 0)
        COLOR_OPTS = {"Black": "#000000", "Yellow": "#FFD700", "White": "#FFFFFF", "Orange": "#FF4500", "Grey": "#AAAAAA"}
    ...
    with st.expander("📍 위치/크기 조절"):
        rx, ry = st.number_input("박스 X", 0, 1080, 40), st.number_input("박스 Y", 0, 1920, 250 if box_orient=="Horizontal" else 1200)
        ...
        vis_sz_adj, vis_alpha = st.slider("지도/그래프 크기", 50, 1080, 1080), st.slider("지도/그래프 투명도", 0, 255, 255)
        ...
        try:
            ...
            if show_box:
                ...
                if box_orient == "Vertical":
                    ...
                    draw_styled_text(draw, (rx + 40, ry + 120), v_date, f_d, "#AAAAAA", shadow=use_shadow)
                    y_c = ry + 200
                    ...
                    tw = draw.textlength(v_act, f_t)
                    draw_styled_text(draw, (rx + (rw-tw)//2, ry+35), v_act, f_t, m_color, shadow=use_shadow)
                    draw_styled_text(draw, (rx + (rw-draw.textlength(v_date, f_d))//2, ry+125), v_date, f_d, "#AAAAAA", shadow=use_shadow)
                    sec_w = rw // 4; y_top = ry + 190
                    for i, (lab, val, diff) in enumerate(items):
                        cx = rx + (i * sec_w) + (sec_w // 2)
                        draw_styled_text(draw, (cx - draw.textlength(lab.lower(), f_l)//2, y_top), lab.lower(), f_l, "#AAAAAA", shadow=use_shadow)
                        draw_styled_text(draw, (cx - draw.textlength(val.lower(), f_n)//2, y_top + 45), val.lower(), f_n, sub_color, shadow=use_shadow)
                        if diff: draw_styled_text(draw, (cx - draw.textlength(diff, f_l)//2, y_top + 110), diff, f_l, m_color, shadow=use_shadow)

            if show_vis:
                if mode == "DAILY" and a.get('map', {}).get('summary_polyline'):
                    pts = polyline.decode(a['map']['summary_polyline']); lats, lons = zip(*pts)
                    vis_sz = vis_sz_adj; vis_layer = Image.new("RGBA", (vis_sz, vis_sz), (0,0,0,0)); m_draw = ImageDraw.Draw(vis_layer)
                    ...
                    vis_sz = vis_sz_adj; vis_layer = chart_img.resize((vis_sz, int(chart_img.size[1]*(vis_sz/chart_img.size[0]))), Image.Resampling.LANCZOS)
            ...
            if log_file:
                ...
                overlay.paste(l_img, (CW - ls - 40, 40), l_img)
