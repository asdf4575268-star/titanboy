...
if not st.session_state.get('access_token'):
    ...
    with st.expander("📂 1. 데이터 및 사진 설정", expanded=True):
        ...
        if acts:
            if mode == "DAILY":
                ...
                if selected_act:
                    v_type = selected_act.get('type', 'Run')
    ...
    with st.expander("🎨 2. 디자인 및 텍스트 수정", expanded=False):
        ...
        if mode != "DAILY" or (mode == "DAILY" and temp_sel == "수동 설정 (Custom)"):
            ...
            with c_opt2: 
                sel_font = st.selectbox("폰트", ["BlackHanSans", "KirangHaerang", "Lacquer", "Condiment", "Bangers", "BagelFatOne"], index=2 if v_type in ["WeightTraining", "Workout"] else 4)
    ...
    if (mode == "DAILY" and selected_act) or (mode == "WEEKLY" and weekly_data) or (mode == "MONTHLY" and monthly_data):
        try:
            ...
            if show_box:
                ...
                if mode == "DAILY":
                    daily_icon = get_icon_pil("dumbbell" if v_type in ["WeightTraining", "Workout"] else "run", size=(60, 60))
                    if daily_icon:
            ...
            if show_vis:
                ...
                elif mode == "DAILY" and v_type not in ["WeightTraining", "Workout"] and selected_act and selected_act.get('map', {}).get('summary_polyline'):
