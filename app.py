...
COLOR_OPTIONS = {"White": "#FFFFFF", "Black": "#000000", "Yellow": "#FFD700", "Orange": "#FF4500"}
...
with col1:
    ...
    bg_files = st.file_uploader("사진 (여러 장 가능)", type=['jpg','jpeg','png'], accept_multiple_files=True)
...
with col3:
    st.header("🎨 DESIGN")
    insta_mode = st.selectbox("비율", ["1:1 (Square)", "4:5 (Portrait)"])
    box_orient = st.radio("방향", ["Vertical", "Horizontal"], horizontal=True)
    sel_font = st.selectbox("폰트", ["BlackHanSans", "Jua", "DoHyeon"])
    m_color = COLOR_OPTIONS[st.selectbox("메인 색상", list(COLOR_OPTIONS.keys()))]
    sub_color = COLOR_OPTIONS[st.selectbox("숫자 색상", list(COLOR_OPTIONS.keys()))]
    box_alpha = st.slider("투명도", 0, 255, 120)
...
if bg_files:
    try:
        ...
        if box_orient == "Horizontal":
            ...
            draw.text((rx+40, ry+150), v_date, font=f_d, fill="#AAAAAA")
            curr_y = ry + 220
            for lab, val in items:
                draw.text((rx+40, curr_y), lab, font=f_l, fill="#AAAAAA")
                draw.text((rx+40, curr_y+30), val, font=f_n, fill=sub_color)
                curr_y += 125
