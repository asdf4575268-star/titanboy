...
def make_smart_collage(files, target_size):
    ...
    for i, img in enumerate(imgs):
        ...
        x2, y2 = int((c + 1) * current_w_step) if (c + 1) < current_cols else tw, int((r + 1) * h_step) if (r + 1) < rows else th
    ...
    try:
        ...
        if st.toggle("데이터 박스 보기", value=True):
            ...
            if box_orient == "Vertical":
                ...
                t_x, t_y = (1080 - title_w)//2, ry + 35
                draw_styled_text(draw, (t_x, t_y), v_act, f_t, m_color, use_shadow)
